...
public class GridDesigner extends PApplet {
...
int gridLineWidth = 1;
int gridLineColor = color(192, 192, 192, 128);
...
public void _drawEllipseDot(int layerId) {
  ...
  for(int i=1; i<displayPixels+1; i++) {
    ...
    for(int j=1; j<displayPixels+1; j++) {
      ...
      if (frameCanvas[layerId][i][j] != 0) {
        ...
        ellipse((x+0.5f)*squareSize + l.posX, (y+0.5f)*squareSize + l.posY, l.dotSize, l.dotSize);
...
public void _drawJointLine(int layerId) {
    ...
    for(int j=1; j<displayPixels+1; j++) {
      ...
      if (frameCanvas[layerId][i][j] != 0) {
        ...
        if (frameCanvas[layerId][i-1][j-1] != 0) {
          ...
          line((x+0.5f)*squareSize + l.posX, (y+0.5f)*squareSize + l.posY, (x-1+0.5f)*squareSize + l.posX, (y-1+0.5f)*squareSize + l.posY);
        ...
        if (drawFlag == false) {
          ...
          noStroke();
          fill(c);
          ellipse((x+0.5f)*squareSize + l.posX, (y+0.5f)*squareSize + l.posY, 3, 3);
...
public void drawGrid(int x, int y, int w, int h, int squareNumX, int squareNumY) {
...
public void _updateGuiByLayerInfo() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "GridDesigner" };
    if (passedArgs != null) {
